...
<body>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <div class="card h-100">
                            ...
                            <img src="{{ asset($category->image_path) }}" class="card-img-top" alt="{{ $category->name }}" style="height: 150px; object-fit: cover;">
                            @endif
                            <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($product->image_path)
                                        <img src="{{ asset($product->image_path) }}" class="img-thumbnail" alt="{{ $product->name }}">
                                    @else
                                        <span class="badge bg-warning text-dark">بدون صورة</span>
                                    @endif
                                </td>
                                <td>{{ $product->name }}</td>
    ...
    <script>
        // تفعيل جميع النوافذ المنبثقة
        var updateImageModals = document.querySelectorAll('.modal');
        updateImageModals.forEach(function(modal) {
            modal.addEventListener('show.bs.modal', function (event) {
                var button = event.relatedTarget;
                var modalTitle = modal.querySelector('.modal-title');
                modalTitle.textContent = 'تحديث صورة المنتج';
            });
        });

        // إظهار رسالة التأكيد عند الحذف
